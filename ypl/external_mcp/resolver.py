...
async def _allowed_servers_for(
    ...
    *,
    ...
    for srv in servers:
        ...
        allowed_roles = {r for r in role_rows}
        ...
        agent_rows = (
            await session.exec(
                select(McpServerAgent.agent_id).where(
                    McpServerAgent.mcp_server_id == srv.mcp_server_id
                )
            )
        ).all()
        allowed_agents = {r for r in agent_rows}
    ...
    async with get_async_session() as session:
        ...
        for slug in requested_slugs:
            if slug not in allowed_slugs:
                unavailable.append({"slug": slug, "reason": "not allowed or disabled"})
        ...
        for srv in servers:
            ...
            if bearer is None:
                unavailable.append(
                    {"slug": srv.slug, "reason": "no active grant — connect at /my_mcps"}
                )
                continue
