...
async def _revoke_existing_active(
    session: AsyncSession, user_id: str, mcp_server_id: uuid.UUID
) -> None:
    ...
    insert a new one."""
    stmt = select(McpUserGrant).where(
        sa.and_(
            McpUserGrant.user_id == user_id,
            McpUserGrant.mcp_server_id == mcp_server_id,
...
async def resolve_bearer(
    ...
    *,
    ...
    needs_refresh = (
        ...
        and grant.expires_at - datetime.now(timezone.utc) < _REFRESH_SAFETY_WINDOW
    )
    if needs_refresh:
        secrets = await session.get(McpServerSecrets, server.mcp_server_id)
