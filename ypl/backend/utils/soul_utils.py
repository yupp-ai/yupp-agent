...
from ypl.db.soul_rbac import RoleName, RolePermission, SoulPermission, SoulRole, UserRole
...
async def has_role(user_id: str, role_name: RoleName, session: AsyncSession) -> bool:
    ...
    """
    try:
        role_stmt = select(SoulRole).where(SoulRole.name == role_name)
...
async def has_permission_cached(creator_user_email: str, permission: SoulPermission) -> bool:
    """
...
@redis_async_cache(ttl_seconds=60 * 5)  # 5 minutes
async def has_permission_by_user_id_cached(user_id: str, permission: SoulPermission) -> bool:
...
async def has_permission_by_user_id(user_id: str, permission: SoulPermission) -> bool:
    """Check if a user has a specific permission based on their user_id.
...
async def _check_user_permission(session: AsyncSession, user_id: str, permission: SoulPermission) -> bool:
    """Check if a user has a specific permission given their user_id and an open DB session."""
    # Get all roles for the user
    roles_stmt = select(UserRole.role_id).where(UserRole.user_id == user_id)
