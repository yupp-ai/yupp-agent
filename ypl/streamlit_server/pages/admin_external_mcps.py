...
from ypl.external_mcp.catalog import CATALOG, CatalogEntry
...
async def _set_server_roles(mcp_server_id: uuid.UUID, role_ids: list[uuid.UUID]) -> None:
    async with get_async_session() as session:
        existing = (
            await session.exec(select(McpServerRoleAccess).where(McpServerRoleAccess.mcp_server_id == mcp_server_id))
        ).all()
        want = set(role_ids)
        ...
        if picked:
            ...
            with col1:
                ...
                if new_enabled != srv["enabled"]:
                    run_coroutine_in_lit_worker(_toggle_enabled(mcp_server_id, new_enabled), timeout=10)
                    st.cache_data.clear()
                    st.rerun()
            ...
            async def _current_role_ids(mid: uuid.UUID = mcp_server_id) -> list[uuid.UUID]:
                async with get_async_session_read_replica() as session:
                    rows = (
                        await session.exec(
                            select(McpServerRoleAccess.role_id).where(McpServerRoleAccess.mcp_server_id == mid)
                        )
                    ).all()
                return list(rows)
            ...
            if set(sel) != current_set:
                if st.button("Save role assignments"):
                    run_coroutine_in_lit_worker(_set_server_roles(mcp_server_id, list(sel)), timeout=10)
                    st.success("Saved.")
                    st.cache_data.clear()
                    st.rerun()
...
with tab_catalog:
    ...
    admin_role_id = next((r.role_id for r in catalog_roles if r.name == RoleName.ADMIN), None)
    ...
    for entry in CATALOG:
        ...
        with st.container(border=True):
            ...
            with head_l:
                ...
                st.markdown(f"**Auth:** {entry.auth_type.value} · **Scopes:** {', '.join(entry.scopes) or '—'}")
                if entry.setup_hint:
            ...
            if not is_registered:
                ...
                if env_cid and env_csec and admin_role_id:
                    ...
                    with st.form(f"catalog_add_{entry.slug}"):
                        cid = st.text_input("OAuth client_id", value=env_cid, key=f"cid_{entry.slug}")
                        csec = st.text_input(
                            "OAuth client_secret",
                            type="password",
                            value=env_csec,
...
with tab_add:
    ...
    with st.form("add_mcp_server", clear_on_submit=False):
        ...
        transport = st.selectbox("Transport", [t.value for t in McpTransport], index=0)
        ...
        oauth_client_id = st.text_input("OAuth client_id", disabled=auth_type != "OAUTH_OBO")
