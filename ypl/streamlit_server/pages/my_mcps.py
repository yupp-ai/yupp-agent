...
async def _current_user_id(email: str) -> str | None:
    async with get_async_session_read_replica() as session:
        row = (await session.exec(select(User.user_id).where(User.email == email))).first()
        return row
...
async def _visible_servers(user_id: str) -> list[dict]:
    ...
    async with get_async_session_read_replica() as session:
        ...
        user_roles = set(
            (
                await session.exec(
                    select(UserRoleAssociation.role_id).where(UserRoleAssociation.user_id == user_id)
                )
            ).all()
        )
        servers = (
...
async def _set_m2m(user_id: str, mcp_server_id: uuid.UUID, api_key: str) -> None:
    async with get_async_session() as session:
        await grants_svc.upsert_m2m_grant(
            session, user_id=user_id, mcp_server_id=mcp_server_id, api_key=api_key
        )
        await session.commit()
...
if not user_id:
    st.warning(
        f"No user record for {email}. Ask an admin to create one on **Users → Add User**."
    )
    st.stop()
...
def _grant_status_badge(grant: McpUserGrant | None) -> tuple[str, str]:
    ...
    if grant.expires_at is not None:
        remaining = grant.expires_at - datetime.now(timezone.utc)
...
try:
    ...
    _lit_base = (st.context.headers.get("origin") or "").rstrip("/")  # type: ignore[attr-defined]
except Exception:
    _lit_base = ""
if not _lit_base:
    ...
    with st.container(border=True):
        ...
        with c2:
            if srv.auth_type == McpAuthType.OAUTH_OBO:
                connect_url = (
                    f"{_AHS_BASE}/mcp_oauth/start"
                    f"?server={srv.slug}&user_id={user_id}"
                ...
                if grant:
                    if st.button("Revoke", key=f"revoke_{srv.slug}"):
                        run_coroutine_in_lit_worker(
                            _revoke(user_id, srv.mcp_server_id), timeout=10
                        )
