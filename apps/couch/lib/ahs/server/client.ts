...
const ahsArtifactSchema = z.object({
  ...
  named_slug: z.string().nullable().optional(),
  created_at: z.string(),
...
const ahsListArtifactsResponseSchema = z.object({
  artifacts: z.array(ahsArtifactSchema),
})

export type AhsArtifact = z.infer<typeof ahsArtifactSchema>
export type AhsListArtifactsResponse = z.infer<
  typeof ahsListArtifactsResponseSchema
>
...
export async function listArtifacts(params: {
  ...
  limit?: number
  offset?: number
}): Promise<AhsListArtifactsResponse> {
  return ahsFetch<AhsListArtifactsResponse>(
    ...
    ahsListArtifactsResponseSchema
  )
}

/**
 * Stream raw artifact bytes — used by the auth-checked proxy route at
 * /api/ahs/artifact/[id]/content. Returns the raw response so the route
 * handler can pipe body, content-type, and content-disposition.
 */
export async function readArtifactContent(artifactId: string): Promise<{
  ...
  const apiKey = getAhsApiKey()
  const url = `${baseUrl()}/ahs/artifacts/${encodeURIComponent(artifactId)}`
  const res = await fetch(url, {
