...
import { getWsUrlAction } from '@/app/_actions/get-ws-url'
...
export function useAhsSession({
  ...
  const connect = useCallback(
    async (sid: string) => {
      ...
      let wsUrl: string
      try {
        wsUrl = await getWsUrlAction(sid)
      } catch {
  ...
  const sendMessage = useCallback(
    ...
    [resubscribeWs, sendWsMessage, session.user.id, sessionId]
  )

  const stop = useCallback(() => {
    if (sessionId && !sendWsMessage({ type: 'stop', session_id: sessionId })) {
      void stopSessionAction(sessionId)
    }
