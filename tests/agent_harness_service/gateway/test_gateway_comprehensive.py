...
# ===========================================================================
# SlackGateway — send_status_update
# ===========================================================================
...
class TestSendStatusUpdate:
    ...
    async def test_success(self, gateway: SlackGateway) -> None:
        gateway._client = _mock_client(_ok_response())
        assert await gateway.send_status_update(SESSION_ID, "Working…") is True
    ...
    @pytest.mark.asyncio
    async def test_session_not_found_evicts_session(self, gateway: SlackGateway) -> None:
        gateway._client = _mock_client(_fail_response("Session not found"))
        result = await gateway.send_status_update(SESSION_ID, "text")
        assert result is False
        assert SESSION_ID in gateway._dead_session_ids
    ...
    @pytest.mark.asyncio
    async def test_other_failure_does_not_evict(self, gateway: SlackGateway) -> None:
        gateway._client = _mock_client(_fail_response("internal error"))
        result = await gateway.send_status_update(SESSION_ID, "text")
        assert result is False
        assert SESSION_ID not in gateway._dead_session_ids
