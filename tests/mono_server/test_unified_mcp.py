...
Verifies:
  ...
  3. PlatformMcpAuthMiddleware:
       - Missing / non-dev token → 401
       - Bearer yupp_dev_* without appdb → 503 (graceful degradation)
       - DB error → 503
...
def _make_platform_app() -> FastAPI:
    from ypl.mono_server.unified_mcp import PlatformMcpAuthMiddleware

    app = FastAPI()
    app.add_middleware(PlatformMcpAuthMiddleware)
...
class TestSplitMcpModule:
    def test_apps_importable(self) -> None:
        ...
        assert platform_mcp_http_app is not None
        assert harness_mcp_http_app is not platform_mcp_http_app

    def test_middlewares_importable(self) -> None:
        from ypl.mono_server.unified_mcp import HarnessMcpAuthMiddleware, PlatformMcpAuthMiddleware

        assert HarnessMcpAuthMiddleware is not None
        assert PlatformMcpAuthMiddleware is not None
    ...
    def test_harness_app_carries_harness_middleware(self) -> None:
        ...
        assert "PlatformMcpAuthMiddleware" not in names, f"got {names}"

    def test_platform_app_carries_platform_middleware(self) -> None:
        from ypl.mono_server.unified_mcp import platform_mcp_http_app

        names = {getattr(m.cls, "__name__", "") for m in platform_mcp_http_app.user_middleware}
        assert "PlatformMcpAuthMiddleware" in names, f"got {names}"
...
class TestHarnessAuth:
    ...
    def test_dev_token_rejected_on_harness_path(self) -> None:
        """yupp_dev_* tokens are for /mcp/platform; harness must reject them."""
        client = TestClient(_make_harness_app(), raise_server_exceptions=False)
...
class TestPlatformAuth:
    def test_no_auth_returns_401(self) -> None:
        client = TestClient(_make_platform_app(), raise_server_exceptions=False)
        assert client.get("/ping").status_code == 401

    def test_ahs_token_rejected_on_platform_path(self) -> None:
        """x-ahs-token is for /mcp/harness; platform must reject it."""
        from ypl.agent_harness_service.common.constants import AHS_MCP_SECRET
    ...
    def test_bearer_secret_colon_session_rejected(self) -> None:
        """Agent-style Bearer is rejected on platform path."""
        from ypl.agent_harness_service.common.constants import AHS_MCP_SECRET
    ...
    def test_dev_token_without_appdb_never_returns_200_or_500(self) -> None:
        """In one-box / CI mode appdb is unavailable — must be 401 or 503."""
        client = TestClient(_make_platform_app(), raise_server_exceptions=False)
        r = client.get("/ping", headers={"authorization": "Bearer yupp_dev_XXXXXXXXXXXXXXXXXXXX"})
    ...
    def test_dev_token_without_use_mcp_returns_403(self) -> None:
        """Mono ``PlatformMcpAuthMiddleware`` must enforce ``USE_MCP``,
...
# ---------------------------------------------------------------------------
# 4. Deprecation header on /mcp/platform
# ---------------------------------------------------------------------------


class TestPlatformDeprecationHeader:
    """``/mcp/platform`` stamps ``X-Auth-Deprecation`` only on responses
    ...
    def test_header_absent_on_non_dev_token_bearer(self) -> None:
        """OAuth JWT misrouted to /mcp/platform → 401, but no deprecation
    ...
    def test_header_absent_on_ahs_token_misroute(self) -> None:
        """``x-ahs-token`` agent-secret on platform → 401 with no deprecation
    ...
    def test_header_present_on_dev_token_unauthorized(self) -> None:
        """``Bearer yupp_dev_*`` (invalid / no-appdb) → header is set so the
        ...
        # 401 (invalid token) or 503 (appdb unavailable) — both must carry the header
        assert r.status_code in {401, 503}, f"got {r.status_code}: {r.text}"
        assert r.headers.get(DEPRECATION_HEADER) == DEPRECATION_NOTICE

    def test_header_present_on_db_error(self) -> None:
        """503 (appdb error) on a dev-token request → deprecation header
        still set."""
        from ypl.mcp_server.auth_dev_token import DEPRECATION_HEADER, DEPRECATION_NOTICE
...
class TestHarnessNoDeprecationHeader:
    ...
    def test_harness_dev_token_misroute_does_not_carry_header(self) -> None:
        ...
        rejection itself already tells the caller to use /mcp/platform.
